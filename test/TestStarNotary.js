...
it('lets user1 put up their star for sale', async() => {
    ...
    const price_app = await instance.starsForSale.call(starId);
    assert.equal(price_app, starPrice);
    //assert.equal(1, 1);
});
...
it('can add the star name and star symbol properly', async() => {
    ...
    const symbol = await instance.getSymbol();


    assert.equal(name, "Cool Star");
    assert.equal(symbol, "CSTAR");
...
it('lets 2 users exchange stars', async() => {
    ...
    // exchange from user 1
    await instance.exchangeStars(starId1, starId2, {from: user1});
    assert.equal(await instance.ownerOf.call(starId1), user2);
    assert.equal(await instance.ownerOf.call(starId2), user1);

    // exchange back with user 2
